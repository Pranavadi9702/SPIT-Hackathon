...
// Validate and clean chart data to prevent NaN errors in Recharts
function validateChartData(data) {
  if (!Array.isArray(data)) return [];
  return data.map((d) => ({
    ...d,
    actual: typeof d.actual === "number" && !isNaN(d.actual) ? d.actual : 0,
    forecast: 
      d.forecast !== null && 
...
export default function EnhancedOverviewCharts({
  ...
  const lineData = useMemo(() => {
    const cleaned = validateChartData(weeklySalesData);
    if (!smoothForecast) return cleaned;
    return smoothen(cleaned, "forecast");
